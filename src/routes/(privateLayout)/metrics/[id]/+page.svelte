...
<section class="hms-container py-6">
	...
	<div class="relative w-full">
		<div class=" absolute top-0 right-0 flex flex-col gap-2">
			<button onclick="{() => (modalDelete = true)}" type="button" class=" btn btn-error">Delete Metrics</button>
			<button type="button" class=" btn btn-success">Email Metrics</button>
		</div>
...
<dialog id="my_modal_1" class="modal-open modal">
	<div class="relative modal-box" transition:fade>
		<button onclick="{() => (modalDelete = false)}" type="button" aria-label="close" class=" absolute top-[15px] right-[15px] cursor-pointer">
			<svg
		...
		<form>
			<p class=" mb-[20px] border-b-[3px] border-[#d7d7d7] pb-[7px] text-lg font-bold text-primary">
				Type "delete metric" and submit.
			</p>

			<input type="text" placeholder="Type Here" class="input input-lg w-full max-w-[203px]" />

			<div class="mt-[45px] flex w-full gap-[26px]">
				<button onclick="{() => (modalDelete = false)}" type="button" class=" btn btn-outline btn-lg btn-primary">Cancel</button>
